...
export function CatalogBulkImport({
  ...
  useEffect(() => {
    ...
    if (importJob.status === "FAILED") {
      notify.error(`${label}: importação falhou. Veja detalhes no sininho.`);
    }
  ...
  const handleFile = async (file: File | undefined) => {
    ...
    try {
      ...
      if (result.validation_errors?.length) {
        notify.error("Planilha com erros de validação. Revise o template.");
        return;
      }

      setImportId(result.import.id);
      notify.success(`${label} iniciado. Acompanhe pelo sininho de notificações.`);
    } catch (err) {
      notify.fromApiError(err, `Não foi possível importar ${label.toLowerCase()}.`);
    } finally {
      if (inputRef.current) {
  ...
  return (
    <div className="rounded-app border border-border bg-surface p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-medium text-foreground">Importação em massa</p>
      ...
      {importJob ? (
        <p className="mt-3 text-xs text-muted-foreground">
          Importação {importJob.id.slice(0, 8)}: {importJob.status} ·{" "}
