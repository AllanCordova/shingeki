...
Extensao **Chrome / Edge (Manifest V3)** que captura cookies HttpOnly do dominio do alvo e completa o ticket publico `POST /api/target-session/capture/{ticket}`.

Nao usa o token Sanctum do usuario — so o ticket de curta duracao armado pelo Shingeki web.
...
```powershell
pwsh ./shingeki-extension/pack.ps1
```
...
Devs ainda podem carregar a pasta `shingeki-extension` direto do repo.
...
| `pack.ps1` | Gera o ZIP em `shingeki-client/public/extensions/` |
