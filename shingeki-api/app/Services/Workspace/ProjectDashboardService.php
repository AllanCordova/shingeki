...
class ProjectDashboardService
{
    ...
    public function build(Project $project): array
    {
        ...
        $ranked = $this->completedDispatchesBySystem($systemIds);
        $latestBySystem = $ranked->map(fn ($group) => $group->first())->filter();
        $previousBySystem = $systemIds->mapWithKeys(function (string $systemId) use ($ranked) {
            return [$systemId => $ranked->get($systemId, collect())->get(1)];
        });
     ...
     */
    private function completedDispatchesBySystem(Collection $systemIds): Collection
    {
