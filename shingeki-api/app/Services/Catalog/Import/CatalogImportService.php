...
class CatalogImportService
{
    ...
    public function processMessage(array $payload): CatalogImport
    {
        ...
        $expectedEvent = $import->type === CatalogImportType::Attacks
            ? 'catalog.attacks.import.batch'
            : 'catalog.remediations.import.batch';

        if (is_string($event) && $event !== $expectedEvent) {
            throw new RuntimeException('Catalog import event does not match the import type.');
        }
        ...
        return DB::transaction(function () use ($import, $userId, $items, $chunkIndex, $isLastChunk): CatalogImport {
            $import = CatalogImport::query()->lockForUpdate()->findOrFail($import->id);
            $processedChunks = $import->processed_chunks ?? [];

            if (in_array($chunkIndex, $processedChunks, true)) {
                return $import;
            }
